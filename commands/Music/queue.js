...
module.exports = {
  ...
  async exec(client, message, args) {
    ...
    return message.channel.send(new Discord.MessageEmbed().setTimestamp().setColor(client.colors.info).setAuthor(`${message.author.tag} | Queue`, message.author.displayAvatarURL()).setTitle(`Now playing: ${parseSongName(songs[0].title, songs[0].author)}`).setDescription(songs.slice(1).map((song, index) => `**${index + 1}**: ${parseSongName(song.title, song.author)}`)));
  }
}
